...
export default class Spline
{
  ...
  static Constant(c: number, x1: number, x2: number): Spline
  {
  ...
  {
    let s = 0;
    while ((s < this.segments.length - 1) && (this.segments[s + 1].x < x)) s++;
    x -= this.segments[s].x;
  ...
  X(y: number, accuracy: number, approxx: number = null)
  {
    ...
    for (let i=0; i<1000; i++) {
      ...
      const nx = x - (ty / dy);
      if (Math.abs(y / dy) < accuracy) return nx;
      x = nx;
    }
    return x;
  }
