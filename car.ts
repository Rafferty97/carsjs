...
export enum LCDirection { Left, Right };
...
export default class Car
{
  ...
  lcdir: number;
  conditions: any[];
  _bc: { x: number, v: number, fx: number, rx: number };
  _bc_valid: boolean;
  lcspline: Spline;

  constructor(lane: Lane, x: number = 0, v: number = 0)
  {
  ...
  testAmber(x: number, coord: string)
  {
  ...
  calcCondAcc(cond, x, fx, rx, v)
  {
    switch (cond.type) {
      ...
      case 'merge': {
        ...
        if (overtake) return Math.max(a2, a1);
  ...
  changeLanes(direction: LCDirection)
  {
    if (this.lane.lc) return;
  ...
  get bc()
  {
    if (!this._bc_valid) this.recalcBaseCoords();
    return this._bc;
  }
  ...
  step(dt: number)
  {
    ...
    // Integrate
    if (a < -this.maxdec) a = -this.maxdec;
    this.v += a * dt;
