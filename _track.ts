...
export default class Track
{
  ...
  cars: Car[];
  lc_cars: Car[];
  conditions: Map<Car, any[]>;
  baseCoords: Map<Car, any>;

  constructor(curve: Curve, latspline: Spline, reverse: boolean = false)
  {
  ...
  baseFromTrack(s: number, dsdt: boolean = false)
  {
    if (s > this.length) {
  ...
  removeCar(car: Car)
  {
    ...
    if (ind != -1) this.cars.splice(ind, 1);
    ind = this.lc_cars.indexOf(car);
    if (ind != -1) this.lc_cars.splice(ind, 1);
  }
  ...
  mapCondToBaseCoords(cond: any)
  {
    if (cond.hasOwnProperty('coord') && cond.coord === 'base')
      return cond;
    if (!cond.hasOwnProperty('x')) return cond;
  ...
  getConditions(car: Car)
  {
    return this.conditions.get(car);
  }
  
  connectTo(track: Track)
  {
