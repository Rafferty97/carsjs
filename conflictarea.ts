...
{
  lanes: { lane: Lane, cars: Car[] }[];
  conflictLanes: boolean[];
  t: number;
  p: number;
  ...
  resetConditions()
  {
    ...
    this.lanes.forEach(l => {
     //l.cars = l.cars.filter(car => car.lane == l.lane);
      if (l.cars.length == 0) recalc = true;
    });
    if (recalc) this.recalcConflicts();
  }
  ...
  calculateConditions()
  {
    ...
    this.lanes.forEach(({ lane, cars }, i) => {
      ...
      for (const carx of lane.incomingCars(0, 'track')) {
        ...
        if (cars.indexOf(car) != -1) { scarx = null; continue; }
        const state: string = 'green';
        // Is giveway or stop?
        if (car.bc.fx < x - 5) break;
        // Is red/amber light?
        if (state == 'red') break;
        if (state == 'amber' && car.testAmber(x, 'track')) break;
        // Is another car conflicting?
        if (this.conflictLanes[i]) break;
      ...
      if (scarx == null) return;
      if (cars.indexOf(scarx.car) != -1) return;
  ...
  recalcConflicts()
  {
    this.conflictLanes = this.lanes.map(() => false);
    this.lanes.forEach(({ lane, cars }, i) => {
      if (cars.length != 0) this.addConflicts(i);
    });
  }
