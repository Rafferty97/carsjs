...
function drawLine(p: (number) => [number, number], s1: number, s2: number, ds: number)
{
  let pos = p(s1);
...
for (let i=0; i<25; i++) cars.push(new Car(track1, 6*i));
for (let i=0; i<15; i++) cars.push(new Car(track2, 6*i));
for (let i=0; i<14; i++) cars.push(new Car(track3, 6*i));
for (let i=0; i<4; i++) cars.push(new Car(track4, 6*i));
