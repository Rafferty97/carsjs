import { default as Spline, SplineCP } from './spline';

export function wrapAngle(ang: number): number
{
...
function getTangentAtMidPoint(points: SplineCP[]): number
{
...
export function getApproxArcLengthParam(
  p: (t: number) => [number, number], t1: number, t2: number, steps: number
  ): { t: (s: number) => number, s: Spline, l: number }
{
...
export function vecDiff(v1: number[], v2: number[])
{
  return v1.map((x, i) => x - v2[i]);
}

export function vecLen(v: number[])
{
  return Math.sqrt(v.reduce((s, x) => s + x*x, 0));
}

export function orientPoints(points: [number, number][], x: number, y: number, a: number): [number, number][]
{
  const mat = [
